...
def compare_correlated_attributes(saved_correlated_attributes: List[str], re_produced_correlated_attributes: List[str]) -> Tuple[bool, List[str]]:
    """
    ...
    for attr in saved_correlated_attributes:
        if attr not in re_produced_correlated_attributes:
            error_messages.append(f"Attribute '{attr}' not found in re-produced correlated attributes, but found in saved results.")
            passed = False
    for attr in re_produced_correlated_attributes:
        if attr not in saved_correlated_attributes:
            error_messages.append(f"Attribute '{attr}' not found in saved correlated attributes, but found in re-produced results.")
            passed = False

    return passed, error_messages


def compare_measure_scores(saved_measure_scores: [Dict[str, List[float]]], re_produced_measure_scores: [Dict[str, List[float]]]) -> Tuple[bool, List[str]]:
    """
    ...
    for measure, scores in saved_measure_scores.items():
        if measure not in re_produced_measure_scores:
            error_messages.append(f"Measure '{measure}' not found in re-produced measure scores, but found in saved results.")
            passed = False
        else:
            for score in scores:
                if score not in re_produced_measure_scores[measure]:
                    error_messages.append(f"Score '{score}' not found in re-produced measure scores for measure '{measure}', but found in saved results.")
                    passed = False
    for measure, scores in re_produced_measure_scores.items():
        if measure not in saved_measure_scores:
            error_messages.append(f"Measure '{measure}' not found in saved measure scores, but found in re-produced results.")
            passed = False
        else:
            for score in scores:
                if score not in saved_measure_scores[measure]:
                    error_messages.append(f"Score '{score}' not found in saved measure scores for measure '{measure}', but found in re-produced results.")
                    passed = False


    return passed, error_messages

def compare_score_dicts(saved_scores: Dict[str, Tuple[str, float]], re_produced_scores: Dict[str, Tuple[str, float]]) -> Tuple[bool, List[str]]:
    """
    ...
    for k, v in saved_scores.items():
        if k not in re_produced_scores:
            ...
            if tuple(v)[1] != re_produced_scores[k][1]:
                error_messages.append(f"Value '{v[1]}' for key '{k}' does not match re-produced value '{re_produced_scores[k][1]}'.")
                passed = False
    for k, v in re_produced_scores.items():
        if k not in saved_scores:
            ...
            if v[1] != tuple(saved_scores[k])[1]:
                error_messages.append(f"Value '{v[1]}' for key '{k}' does not match saved value '{saved_scores[k][1]}'.")
                passed = False

    return passed, error_messages


def compare_influence_vals(saved_influence_vals: List[Dict], re_produced_influence_vals: List[Dict]) -> Tuple[bool, List[str]]:
    """
            ...
            if saved_column == re_column and saved_bin == re_bin:
                ...
                for v in values:
                    if v not in re_values and v not in seen_errors[(saved_column, saved_bin)]:
                        error_messages.append(f"Value '{v}' not found in re-produced influence values for column '{saved_column}' and bin '{saved_bin}', but found in saved results.")
                        passed = False
                        seen_errors[(saved_column, saved_bin)].add(v)
                for v in re_values:
                    if v not in values and v not in seen_errors[(saved_column, saved_bin)]:
                        error_messages.append(f"Value '{v}' not found in saved influence values for column '{re_column}' and bin '{re_bin}', but found in re-produced results.")
                        passed = False
                        seen_errors[(saved_column, saved_bin)].add(v)
                break
        if not found:
            error_messages.append(f"Column '{saved_column}' and bin '{saved_bin}' not found in re-produced influence values, but found in saved results.")
            passed = False
    ...
    for vals in re_produced_influence_vals:
        ...
        for saved_vals in saved_influence_vals:
            ...
            if re_column == saved_column and re_bin == saved_bin:
                ...
                break
        if not found:
            error_messages.append(f"Column '{re_column}' and bin '{re_bin}' not found in saved influence values, but found in re-produced results.")
            passed = False

    return passed, error_messages

def compare_significance_vals(saved_significance_vals: List[Dict], re_produced_significance_vals: List[Dict]) -> Tuple[bool, List[str]]:
    """
        ...
        for re_vals in re_produced_significance_vals:
            ...
            if saved_column == re_column and saved_bin == re_bin:
                ...
                if saved_significance != re_significance and (saved_significance, re_significance) not in seen_errors[(saved_column, saved_bin)]:
                    error_messages.append(f"Saved significance '{saved_significance}' does not match re-produced significance '{re_significance}' for column '{saved_column}' and bin '{saved_bin}'.")
                    passed = False
                    seen_errors[(saved_column, saved_bin)].add((saved_significance, re_significance))
                break
        if not found:
            error_messages.append(f"Column '{saved_column}' and bin '{saved_bin}' not found in re-produced significance values, but found in saved results.")
            passed = False
        ...
        for saved_vals in saved_significance_vals:
            ...
            if re_column == saved_column and re_bin == saved_bin:
                ...
                if re_significance != saved_significance and (saved_significance, re_significance) not in seen_errors[(re_column, re_bin)]:
                    error_messages.append(f"Re-produced significance '{re_significance}' does not match saved significance '{saved_significance}' for column '{re_column}' and bin '{re_bin}'.")
                    passed = False
                    seen_errors[(re_column, re_bin)].add((saved_significance, re_significance))
                break
        if not found:
            error_messages.append(f"Column '{re_column}' and bin '{re_bin}' not found in saved significance values, but found in re-produced results.")
            passed = False
    ...
    if saved_results.shape != re_produced_results.shape:
        ...
        for i in range(len(saved_results.shape)):
            if saved_results.shape[i] != re_produced_results.shape[i]:
                error_dims.append(i)
        error_messages.append(f"Saved results shape '{saved_results.shape}' does not match re-produced results shape '{re_produced_results.shape}'. Inconsistent dimensions: {error_dims}.")
        passed = False
    else:
...
def compare_one_to_many_attributes(saved_one_to_many_attributes: List[str], re_produced_one_to_many_attributes: List[str]) -> Tuple[bool, List[str]]:
    """
    ...
    for attr in saved_one_to_many_attributes:
        if attr not in re_produced_one_to_many_attributes:
            error_messages.append(f"Attribute '{attr}' not found in re-produced one-to-many attributes, but found in saved results.")
            passed = False
    for attr in re_produced_one_to_many_attributes:
        if attr not in saved_one_to_many_attributes:
            error_messages.append(f"Attribute '{attr}' not found in saved one-to-many attributes, but found in re-produced results.")
            passed = False

    return passed, error_messages

def compare_predicates(saved_predicates: List[Tuple[str, Tuple[float, float] | str, float, str, int | None]], re_produced_predicates: List[Tuple[str, Tuple[float, float] | str, float, str, int | None]]) -> Tuple[bool, List[str]]:
    """
    ...
    if saved_len != re_produced_len:
        error_messages.append(f"Saved predicates length '{saved_len}' does not match re-produced predicates length '{re_produced_len}'.")
        passed = False
    passed = True
    for i in range(len(saved_predicates)):
        try:
            ...
            try:
                ...
                saved_cat_or_bin = tuple(saved_pred[1]) if (isinstance(saved_pred[1], tuple) or isinstance(saved_pred[1], list)) else saved_pred[1]
                re_produced_cat_or_bin = tuple(re_produced_pred[1]) if (isinstance(re_produced_pred[1], tuple) or isinstance(re_produced_pred[1], list)) else re_produced_pred[1]
                ...
                if saved_col != re_produced_col:
                    local_errors.append(f"\t\t- Saved column '{saved_col}' does not match re-produced column '{re_produced_col}'.")
                    num_errors += 1
                if saved_cat_or_bin != re_produced_cat_or_bin:
                    local_errors.append(f"\t\t- Saved category or bin '{saved_cat_or_bin}' does not match re-produced category or bin '{re_produced_cat_or_bin}'.")
                    num_errors += 1
                if not np.isclose(saved_influence, re_produced_influence):
                    local_errors.append(f"\t\t- Saved influence '{saved_influence}' does not match re-produced influence '{re_produced_influence}'.")
                    num_errors += 1
                if saved_type != re_produced_type:
                    local_errors.append(f"\t\t- Saved type '{saved_type}' does not match re-produced type '{re_produced_type}'.")
                    num_errors += 1
                if saved_index != re_produced_index:
                    local_errors.append(f"\t\t- Saved index '{saved_index}' does not match re-produced index '{re_produced_index}'.")
                if num_errors != 0:
                    error_messages.append(f"\t{num_errors} errors in predicate {i}. Saved predicate: {saved_pred}, re-produced predicate: {re_produced_pred}")
                    error_messages += local_errors
                    passed = False
            except IndexError:
                if len(saved_pred) != 5 and len(re_produced_pred) != 5:
                    error_messages.append(f"Saved predicate and re-produced predicate both have a different length than expected. If this is due to an intentional change, please ignore this error or update the comparison function. Saved predicate: {saved_pred}, re-produced predicate: {re_produced_pred}")
                elif len(saved_pred) != 5:
                    error_messages.append(f"Saved predicate {saved_pred} has a different length from expected, and may be deformed.")
                elif len(re_produced_pred) != 5:
                    error_messages.append(f"Re-produced predicate {re_produced_pred} has a different length from expected, and may be deformed.")
                passed = False
    ...
    if len(re_produced_predicates) > len(saved_predicates):
        error_messages.append("Re-produced predicates are longer than saved predicates. Additional predicates found in re-produced results.")
...
def compare_final_inf(saved_final_inf: float, re_produced_final_inf: float) -> Tuple[bool, List[str]]:
    ...
    if saved_final_inf != re_produced_final_inf:
        error_messages.append(f"Saved final influence value '{saved_final_inf}' does not match re-produced final influence value '{re_produced_final_inf}'.")
        passed = False

    return passed, error_messages

def compare_final_predicate(saved_final_pred: List[Tuple[str, Tuple[float, float] | str, float, str, int | None]], re_produced_final_pred: List[Tuple[str, Tuple[float, float] | str, float, str, int | None]]) -> Tuple[bool, List[str]]:
    """
    ...
    if saved_final_pred != re_produced_final_pred:
        error_messages.append(f"Saved final predicate '{saved_final_pred}' does not match re-produced final predicate '{re_produced_final_pred}'.")
        passed = False
...
def compare_final_df(saved_final_df: DataFrame, re_produced_final_df: DataFrame) -> Tuple[bool, List[str]]:
    ...
    if saved_final_df.shape != re_produced_final_df.shape:
        ...
        for i in range(len(saved_final_df.shape)):
            if saved_final_df.shape[i] != re_produced_final_df.shape[i]:
                error_dims.append(i)
        error_messages.append(f"Saved final DataFrame shape '{saved_final_df.shape}' does not match re-produced final DataFrame shape '{re_produced_final_df.shape}'. Inconsistent dimensions: {error_dims}.")
        passed = False
    ...
    for row in range(saved_final_df.shape[0]):
        ...
        for re_row in range(re_produced_final_df.shape[0]):
            ...
            if saved_row.equals(re_produced_row):
                found = True
                break
        if not found:
            error_messages.append(f"Row {row} with values: {saved_row} not found in re-produced final DataFrame, but found in saved final DataFrame.")
            passed = False
    ...
    for row in range(re_produced_final_df.shape[0]):
        ...
        for saved_row in range(saved_final_df.shape[0]):
            ...
            if re_produced_row.equals(saved_row):
                found = True
                break
        if not found:
            error_messages.append(f"Row {row} with values: {re_produced_row} not found in saved final DataFrame, but found in re-produced final DataFrame.")
            passed = False
